...
**배포는 [DEPLOY.md](DEPLOY.md) 의 순서를 그대로 따라가면 됩니다.**
...
서비스의 얼굴은 마스코트 **등불이**(말씀 길잡이)입니다. 시편 119:105에서 이름을 가져왔고,
히어로 인사 · 상담 챗봇 · 성소의 연속 묵상 · 빈 화면에 같은 인격으로 등장합니다.
전부 인라인 SVG라 이미지 파일이 없고 라이트·다크 모드를 저절로 따라갑니다.
이름을 바꾸려면 `index.html` 마스코트 스크립트 첫머리의 `var NAME` 한 줄만 고치면 됩니다.
표정·크기·적용 위치는 `../등불이_캐릭터시트.html` 에서 한눈에 볼 수 있습니다.
